...
class RecursiveTestCase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.initial_request = factory.post(path=url)

    def test_should_update_initial_request_query_param_value_with_custom_goodbye(
        self,
    ) -> None:
        # Given
        ...
        # When
        request = self.under_test._update_query_param(
            self.query_param, self.initial_request
        )
    ...
    def test_should_log_debug_alert_message_when_making_recursive_call(self) -> None:
        ...
        with self.assertLogs(service_logger, level="DEBUG") as cm:
            self.under_test.make_recursive_call(
                self.query_param, self.initial_request
            )  # When
