...
class GreetingsExceptionsTestCase(TestCase):
    ...
    def test_should_raise_exception_for_absent_query_param_key_in_request_url(
        self,
    ) -> None:
        # Given
    ...
    def test_should_raise_exception_when_query_param_value_is_blank_or_none(
        self,
    ) -> None:
        # Given
        value = ""
        ...
        with self.assertRaisesMessage(
            ValidationError, "Value for query param: greeting cannot be blank or empty."
        ):
            save_custom_greeting(request)  # When

    def test_should_raise_exception_if_query_param_value_has_non_alphabetical_chars(
        self,
    ) -> None:
        # Given
        value = "_greeting@37"
